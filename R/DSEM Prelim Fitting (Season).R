...
# AransasBay_Sciaenid_Wide_Trans_Season 
# AransasBay_Pred_Wide_Trans_Season
# GalvestonBay_Sciaenid_Wide_Trans_Season
# GalvestonBay_Pred_Wide_Trans_Season
...
AB_Sciaenid_TS_Season <- ts(
  AransasBay_Sciaenid_Wide_Trans_Season %>%
    select(-Timestep, -SEASON, -YEAR),  
  start = c(min(AransasBay_Sciaenid_Wide_Trans_Season$Timestep)),  
  end = c(max(AransasBay_Sciaenid_Wide_Trans_Season$Timestep)),
...
fitAB_Sciaenid_Season = dsem( sem = semAB_Sciaenid,
                       tsdata = AB_Sciaenid_TS_Season,
                       control = dsem_control(
                         quiet = TRUE))
summary(fitAB_Sciaenid_Season)
...
coef_matrix_AB_Sciaenid_NoLag_Season <- get_part_Sciaenid(as_fitted_DAG(fitAB_Sciaenid_Season, lag=0))
coef_matrix_AB_Sciaenid_YesLag_Season <- get_part_Sciaenid(as_fitted_DAG(fitAB_Sciaenid_Season, lag=1))

# create an empty data frame with specified column names
df_Sciaenid_Season <- data.frame(Salinity = numeric(),
                          BrownShrimp = numeric(),
...
# make path diagrams
png("sciaenidAB-path-seasonal.png", width = 2000, height = 1300, res = 300)
par(mfrow = c(1, 2))
plot1 <- plot_qgraph_panel(df_Sciaenid_Season, coef_matrix_AB_Sciaenid_NoLag_Season$coef,  plot_title = "Sciaenid-AB-No Lag (Seasonal)")
plot2 <- plot_qgraph_panel(df_Sciaenid_Season, coef_matrix_AB_Sciaenid_YesLag_Season$coef, plot_title = "Sciaenid-AB-Yes Lag (Seasonal)")
par(mfrow = c(1, 1))
dev.off()
...
# Second System and Major Bay: Sciaenid Trophic System and Galveston Bay 

GB_Sciaenid_TS_Season <- ts(
  GalvestonBay_Sciaenid_Wide_Trans_Season %>%
    select(-Timestep, -SEASON, -YEAR),  
  start = c(min(GalvestonBay_Sciaenid_Wide_Trans_Season$Timestep)),  
  end = c(max(GalvestonBay_Sciaenid_Wide_Trans_Season$Timestep)),
  frequency = 1  
)
...
# fit the dsem model for the sciaenid system for aransas bay 
fitGB_Sciaenid_Season = dsem( sem = semGB_Sciaenid,
                       tsdata = GB_Sciaenid_TS_Season,
                       control = dsem_control(
                         quiet = TRUE))
summary(fitGB_Sciaenid_Season)

# extract the coefficient matrix 
coef_matrix_GB_Sciaenid_NoLag_Season <- get_part_Sciaenid(as_fitted_DAG(fitGB_Sciaenid_Season, lag=0))
coef_matrix_GB_Sciaenid_YesLag_Season <- get_part_Sciaenid(as_fitted_DAG(fitGB_Sciaenid_Season, lag=1))

# create an empty data frame with specified column names
df_Sciaenid_GB_Season <- data.frame(Salinity = numeric(),
                             BrownShrimp = numeric(),
                             SpottedSeatrout = numeric(),
                             Temperature = numeric(),
                             SouthernFlounder = numeric(),
...
# make path diagrams
png("sciaenidGB-path-seasonal.png", width = 2000, height = 1300, res = 300)
par(mfrow = c(1, 2))
plot1 <- plot_qgraph_panel(df_Sciaenid_GB_Season, coef_matrix_GB_Sciaenid_NoLag_Season$coef,  plot_title = "Sciaenid-GB-No Lag (Seasonal)")
plot2 <- plot_qgraph_panel(df_Sciaenid_GB_Season, coef_matrix_GB_Sciaenid_YesLag_Season$coef, plot_title = "Sciaenid-GB-Yes Lag (Seasonal)")
par(mfrow = c(1, 1))
dev.off()
...
# Repeating the above but for the...
# Third System and Major Bay: Keystone Predator Trophic System and Aransas Bay 
# get data into a time series object from all numeric columns except YEAR
AB_Pred_TS_Season <- ts(
  AransasBay_Pred_Wide_Trans_Season %>%
    select(-Timestep, -SEASON, -YEAR),  
  start = c(min(AransasBay_Pred_Wide_Trans_Season$Timestep)),  
  end = c(max(AransasBay_Pred_Wide_Trans_Season$Timestep)),
  frequency = 1  # assuming yearly data
)
...
fit_AB_Pred_TS_Season = dsem(sem = semAB_Pred,
                      tsdata = AB_Pred_TS_Season,
                      control = dsem_control(
                        quiet = TRUE))
summary(fit_AB_Pred_TS_Season)
...
# extract the coefficient matrix 
coef_matrix_AB_Pred_NoLag_Season <- get_part_pred(as_fitted_DAG(fit_AB_Pred_TS_Season, lag=0))
coef_matrix_AB_Pred_YesLag_Season <- get_part_pred(as_fitted_DAG(fit_AB_Pred_TS_Season, lag=1))

# create an empty data frame with specified column names
df_Pred_Season <- data.frame(Salinity = numeric(),
...
png("predAB-path-seasonal.png", width = 2000, height = 1300, res = 300)
par(mfrow = c(1, 2))
plot1 <- plot_qgraph_panel(df_Pred_Season, coef_matrix_AB_Pred_NoLag_Season$coef,  plot_title = "Keystone Predator-AB-No Lag (Seasonal)")
plot2 <- plot_qgraph_panel(df_Pred_Season, coef_matrix_AB_Pred_YesLag_Season$coef, plot_title = "Keystone Predator-AB-Yes Lag (Seasonal)")
par(mfrow = c(1, 1))
dev.off()
...
# get data into a time series object from all numeric columns except YEAR
GB_Pred_TS_Season <- ts(
  GalvestonBay_Pred_Wide_Trans_Season %>%
    select(-Timestep, -SEASON, -YEAR),  
  start = c(min(GalvestonBay_Pred_Wide_Trans_Season$Timestep)),  
  end = c(max(GalvestonBay_Pred_Wide_Trans_Season$Timestep)),
  frequency = 1  # assuming yearly data
)
...
# fit the dsem model for the keystone predator system for aransas bay 
fit_GB_Pred_TS_Season = dsem(sem = semGB_Pred,
                      tsdata = GB_Pred_TS_Season,
                      control = dsem_control(
                        quiet = TRUE))
summary(fit_GB_Pred_TS_Season)
...
# extract the coefficient matrix 
coef_matrix_GB_Pred_NoLag_Season <- get_part_pred(as_fitted_DAG(fit_GB_Pred_TS_Season, lag=0))
coef_matrix_GB_Pred_YesLag_Season <- get_part_pred(as_fitted_DAG(fit_GB_Pred_TS_Season, lag=1))

# create an empty data frame with specified column names
df_Pred_GB_Season <- data.frame(Salinity = numeric(),
                         StripedMullet = numeric(),
                         BullShark = numeric(),
                         AlligatorGar = numeric(),
                         Temperature = numeric(),
...
png("predGB-path-seasonal.png", width = 2000, height = 1300, res = 300)
par(mfrow = c(1, 2))
plot1 <- plot_qgraph_panel(df_Pred_GB_Season, coef_matrix_GB_Pred_NoLag_Season$coef,  plot_title = "Keystone Predator-GB-No Lag (Seasonal)")
plot2 <- plot_qgraph_panel(df_Pred_GB_Season, coef_matrix_GB_Pred_YesLag_Season$coef, plot_title = "Keystone Predator-GB-Yes Lag (Seasonal)")
par(mfrow = c(1, 1))
dev.off()
